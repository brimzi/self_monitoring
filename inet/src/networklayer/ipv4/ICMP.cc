...
//This file has been edited to allow our custom protocol to use this infrastructure : Brian Mwambazi
...
void ICMP::handleMessage(cMessage *msg)
{
    ...
    {
        //@Brian Mwambazi: A hack to allow us to build on top ICMP,I know its crazy but have no choice ;)
        PingPayload *pingpkt=check_and_cast<PingPayload *>(msg);

        if(pingpkt->getPktType()==1)
            sendCustomPacket(pingpkt);
        else
        //end of hack
        sendEchoRequest(pingpkt);
        return;
    }
}
...
void ICMP::processEchoReply(ICMPMessage *reply)
{
    ...
    //Brian Mwambazi Hack!!!
        else if(payload->getPktType()==1)
        {
            send(payload, "pingOut",0);

        }
    //Brian Mwambazi End of hack
    else
    {
...
//Brian Mwambazi A hack !
void ICMP::sendCustomPacket(PingPayload *msg)
{
    cGate *arrivalGate = msg->getArrivalGate();
    int i = arrivalGate->getIndex();
    pingMap[msg->getOriginatorId()] = i;
    ...
    ctrl->setProtocol(IP_PROT_ICMP);
    ICMPMessage *request = new ICMPMessage(msg->getName());
    request->setType(ICMP_ECHO_REPLY);
    request->encapsulate(msg);
    request->setControlInfo(ctrl);
    sendToIP(request);
}
